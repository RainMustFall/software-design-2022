...
internal static class Demo
{
    private class Box10X : View
    {
        ...
        public override void Redraw(Rect region)
        {
            ...
            {
                Move(0, y);
                for (var x = 0; x < 10; x++) Driver.AddRune((Rune)('0' + (x + y) % 10));
            }
        }
    }
    ...
    private static void ShowTextAlignments()
    {
    ...
    private static void Main(string[] args)
    {
        ...
        var menu = new MenuBar(new[]
        {
            ...
            {
                new("_New", "Creates new file", NewFile),
                new("_Open", "", null),
                new("_Close", "", Close),
                new("_Quit", "", () =>
                {
            ...
            new("A_ssorted", new MenuItem[]
            {
                new("_Show text alignments", "", ShowTextAlignments, null, null,
                    Key.AltMask | Key.CtrlMask | Key.G)
            }),
            miScrollViewCheck = new MenuBarItem("ScrollView", new MenuItem[]
            {
                new("Box10x", "", ScrollViewCheck) { CheckType = MenuItemCheckStyle.Radio, Checked = true },
                new("Filler", "", ScrollViewCheck) { CheckType = MenuItemCheckStyle.Radio }
            })
        });
        ...
        var statusBar = new StatusBar(new StatusItem[]
        {
            ...
            new(Key.F2, "~F2~ Load", () => MessageBox.Query(50, 7, "Load", "Loading", "Ok")),
            new(Key.F3, "~F3~ Save", () => MessageBox.Query(50, 7, "Save", "Saving", "Ok")),
            new(Key.CtrlMask | Key.Q, "~^Q~ Quit", () =>
            {
                if (Quit()) top.Running = false;
            }),
            new(Key.Null, Application.Driver.GetType().Name, null)
        });
